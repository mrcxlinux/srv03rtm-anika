...
rem eg. a call "rc.bat -i..\inc;..\rinc test.rc" becomes "msdos32 rc.16 -i..\inc ..\rinc test.rc" below, making rc treat ..\rinc as the input filename...
rem in the (rare?) instances of things using rc with ; or =, changing it to call msdos32 directly seems to fix it
...
rem no matter what, it seems rc will eventually error on some file
rem give it 3 chances I guess
if %TRIES% EQU 3 goto end
IF %ERRORLEVEL% NEQ 0 (
  sleep %TIMEOUT%
  goto try
)
