...
PMSIHANDLE g_hDatabase = NULL;
PMSIHANDLE g_hInstall = NULL;
UINT g_iFileCount;
...
int __cdecl _tmain( int argc, TCHAR* argv[], TCHAR* envp[] )
{
	...
	//
	// 4chan: bugfix: app won't exit if this handle isn't closed, under win10 at least
	//
	if ( g_hInstall != NULL )
	{
		MsiCloseHandle(g_hInstall);
	}
	return iRet;
}

//
// 4chan: wrap main code in seperate function, so we can always call MsiCloseHandle above despite whatever it returns
//
UINT MainFn()
{
	//
	...
	UINT iRet = MsiOpenDatabase( g_MSIFile, MSIDBOPEN_READONLY, &g_hDatabase );
...
static bool CreateCABDDF( void )
{
	...
	while( ERROR_NO_MORE_ITEMS != MsiViewFetch( hView, &hRecord ) )
	{
		...
		{
			//
			// 4chan: replace SysWOW64 in the path with System32, as 64-bit machines may return that
			//
			TCHAR* sysWowPos     = szComponentFolder + _tcslen( szWindowsDir ) + 1;
			TCHAR* sysWowTest	 = TEXT("syswow64");
			TCHAR* sysWowReplace = TEXT("system32");
			if( 0 == _tcsnicmp( sysWowPos, sysWowTest, _tcslen( sysWowTest ) ) )
			{
				for( size_t i = 0; i < _tcslen( sysWowReplace ); i++ ) {
					sysWowPos[i] = sysWowReplace[i];
				}
			}
